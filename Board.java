...
import javax.swing.JOptionPane;
	...
	public class Board {
		...
		public int temp = 0;	// used in isBoardEmpty()
		...
		public boolean isBoardEmpty(){
			...
			for (int j =0; j<16; j++)
				{
					if (wordBoard[i][j] != '\0'){
						return false;	//if the board is not blank, temp = 1, need to make sure the centre piece is occupied
					}
		...
		public void placeWord(String s, Frame f, int coordinate1Index1, char coordinate1Index2, int coordinate2Index1, char coordinate2Index2)
		{
			...
			if(checkWord(f, s, coordinate1Index1, coordinate1Index2new, coordinate2Index1, coordinate2Index2new) == true)
					{ 
				System.out.println("checkWord TRUE");
			if(index1Diff==0)
			{
				System.out.println("placeWord TEST 1");
				
				for(int i=0; i<charArray.length && coordinate1Index2new<=coordinate2Index2new; i++) {
					if(wordBoard[coordinate1Index1][coordinate1Index2new] != '\0')
			...
			System.out.println("index2Diff" + index2Diff);
			if(index2Diff==0)
			{
				System.out.println("placeWord TEST 2");
		...
		public boolean checkWord(Frame f, String s, int pos1index1, int pos1index2, int pos2index1, int pos2index2)
		{
			Boolean isOkay=true;
		...
		public boolean checkFrame(Frame f, String word)
		{
			...
			{
				if(f.LetterQuery(charArray[i])==false) {
					System.out.println("FRAMEERROR");
					return false;
				}
		...
		public void finished(){
			String J;
			J = JOptionPane.showInputDialog(null, "Are you finished? Y/N: ");
			if (J== "N"){
				//placeTile();
			}
			if(J=="Y"){
